# WeChatBot ![python](https://img.shields.io/badge/python-2.7-ff69b4.svg)

**WeChatBot** 是用Python包装Web微信协议实现的微信机器人框架。
...
**WeChatBot** 用到了Python **requests** , **pypng** , **Pillow** 以及 **pyqrcode** 库。
...
利用 **WeChatBot** 最简单的方法就是继承WeChatBot类并实现 `handle_msg_all` 或者 `schedule` 函数，然后实例化子类并调用 `run` 方法 。
...
如果运行在Linux下，还可以通过设置 **WeChatBot** 对象的 `conf['qr']` 为 `tty` 的方式直接在终端打印二维码(此方法只能在Linux终端下使用)，效果如下：

![login_on_ubuntu](doc/img/login_on_ubuntu.png)

## 3 效果展示

测试代码 ***test.py*** 的运行效果：

![向机器人发送消息](doc/img/send_msg.png)
...
| `special_list` | 特殊账号列表 |
| `session` | **WeChatBot** 与WEB微信服务器端交互所用的 **Requests** `Session` 对象 |

### 4.6 WeChat 对象方法

**WeChat** 对象还含有一些可以利用的方法
...
***wechatbot.py*** 用 **[图灵机器人](http://www.tuling123.com/)** API 以及 **WeChat** 实现了一个自动回复机器人。
...
- 要接入图灵机器人API时：
  ...
  2. 在 ***wechatbot.py*** 文件所在目录下新建 ***conf.ini*** 文件，内容为:(key字段内容为申请到的图灵key)
